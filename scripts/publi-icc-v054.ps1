...
$WorkRoot = Join-Path $env:TEMP "icc-v054"
...
Write-Host "Script revision: v054-r1 (combat map context)" -ForegroundColor DarkGray
Write-Host "Publicação da v0.5.4 com mapa/cluster em dano, cura e resultados de combate." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v054.iss"
...
if ($projectText -notmatch '<ApplicationVersion>\s*0\.5\.4\.0\s*</ApplicationVersion>') {
    throw "csproj não está em v0.5.4."
}
if ($assemblyText -notmatch 'AssemblyFileVersion\s*\(\s*"0\.5\.4\.0"\s*\)') {
    throw "AssemblyFileVersion não está em v0.5.4."
}
if ($assemblyText -notmatch 'AssemblyInformationalVersion\s*\(\s*"0\.5\.4"\s*\)') {
    throw "AssemblyInformationalVersion não está em v0.5.4."
}
...
if ($bridgeText -notmatch 'ClientVersion\s*=\s*"0\.5\.4"') {
    throw "ClientVersion da telemetria não está em v0.5.4."
}
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.5.4*") {
    throw "Executável gerado não tem FileVersion 0.5.4."
}
...
$notes = @"
### Combate por mapa
- `combat_delta` agora inclui o cluster/mapa atual.
- Kills, deaths, knockouts e knocked_out também carregam o cluster.
- Dano e cura são acumulados separadamente por jogador + mapa, evitando misturar duas zonas dentro da mesma janela de flush.

### Compatibilidade
- Mantém heartbeat, diagnóstico v0.5.3, Guild Presence, party dedup, CTA context, loot e outbox persistente.
- Eventos antigos sem cluster continuam válidos; o War Room os mostra como mapa desconhecido.

### Objetivo
Permitir que o War Room gere Battle Reports da IMORTAIS separados por mapa, com kills, mortes, dano e cura da nossa zerg em cada zona.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.5.4 usa a assinatura
...
Write-Host "==================================================" -ForegroundColor DarkGray
Write-Host ""
Write-Host "TESTE CONTROLADO DO UPDATER v0.5.2 -> v0.5.4" -ForegroundColor Yellow
Write-Host ""
Write-Host "1. NÃO instale a v0.5.4 manualmente na máquina de teste." -ForegroundColor Cyan
...
Write-Host "4. Confirme que a janela oferece a v0.5.4." -ForegroundColor Cyan
Write-Host "5. Atualize e confirme download, fechamento, instalação e relançamento." -ForegroundColor Cyan
Write-Host "6. Após reabrir, confirme v0.5.4 e faça combate em dois mapas para validar a separação por cluster." -ForegroundColor Cyan
Write-Host ""
