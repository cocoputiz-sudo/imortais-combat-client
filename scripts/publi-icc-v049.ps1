...
$WorkRoot = Join-Path $env:TEMP "icc-v049"
...
Write-Host "Publicação da v0.4.9 com Central de Diagnóstico e teste real do updater v0.4.8 -> v0.4.9." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v049.iss"
...
if ($projectText -notmatch '<ApplicationVersion>0\.4\.8\.0</ApplicationVersion>') {
    throw "csproj não está em v0.4.9."
}
if ($assemblyText -notmatch 'AssemblyFileVersion\("0\.4\.8\.0"\)') {
    throw "AssemblyFileVersion não está em v0.4.9."
}
if ($assemblyText -notmatch 'AssemblyInformationalVersion\("0\.4\.8"\)') {
    throw "AssemblyInformationalVersion não está em v0.4.9."
}
...
if ($bridgeText -notmatch 'version = "0.4.9"') {
    throw "Telemetria ainda não anuncia v0.4.9."
}
$UpdaterFile = Join-Path $ProjectDir "Updater\AutoUpdateController.cs"
$updaterText = [System.IO.File]::ReadAllText($UpdaterFile)
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.4.9*") {
    throw "Executável gerado não tem FileVersion 0.4.9."
}
...
- Mostra versão instalada, estado do updater e horário da última checagem.
- Mostra último contato bem-sucedido com o War Room.
- Exibe quantidade e tamanho dos eventos pendentes na outbox.
- Exibe o último erro de telemetria.
- Adiciona o botão **Copiar diagnóstico** para suporte no Discord.

### Verificar atualização
- O botão em Configurações agora mostra visualmente quando a consulta está em andamento.
- Exibe resultado da última checagem: atualizado, atualização disponível ou falha.
- A Home IMORTAIS também possui botão para verificar atualização manualmente.

### Updater
- Mantém detecção desacoplada da janela de startup.
...
Não instale a v0.4.9 manualmente na máquina de teste antes de verificar o fluxo automático.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.4.9 usa a assinatura
...
if (Test-Path -LiteralPath $ReadmeFile) {
    ...
    $readmeText = $readmeText.Replace("> **Versão publicada atual:** v0.4.8", "> **Versão publicada atual:** v0.4.9")
...
Write-Host "==================================================" -ForegroundColor DarkGray
Write-Host ""
Write-Host "TESTE CONTROLADO DO UPDATER v0.4.8 -> v0.4.9" -ForegroundColor Yellow
Write-Host "" 
Write-Host "1. NÃO instale a v0.4.9 manualmente na máquina de teste." -ForegroundColor Cyan
Write-Host "2. Confirme que o Combat Client instalado ainda está em v0.4.8." -ForegroundColor Cyan
Write-Host "3. Abra a v0.4.8 e aguarde a checagem automática; se necessário use Verificar atualização." -ForegroundColor Cyan
Write-Host "4. Confirme que a janela oferece a v0.4.9." -ForegroundColor Cyan
Write-Host "5. Atualize e observe download, fechamento, instalação e relançamento." -ForegroundColor Cyan
Write-Host "6. Após reabrir, confirme v0.4.9 e a nova Central de Diagnóstico." -ForegroundColor Cyan
Write-Host "7. Teste também o botão Verificar atualização em Configurações." -ForegroundColor Cyan
Write-Host ""
