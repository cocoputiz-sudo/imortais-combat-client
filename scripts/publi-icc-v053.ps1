...
$WorkRoot = Join-Path $env:TEMP "icc-v053"
...
Write-Host "Script revision: v053-r1 (runtime diagnostics)" -ForegroundColor DarkGray
Write-Host "Publicação da v0.5.3 com diagnóstico operacional local da telemetria." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v053.iss"
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.5.3*") {
    throw "Executável gerado não tem FileVersion 0.5.3."
}
...
- A Home IMORTAIS agora mostra heartbeat, último party snapshot, quantidade de snapshots repetidos ignorados, Guild Presence observada e detecção do Albion.
- O painel de atividade recente mostra mudanças de party, mudanças de CTA e eventos recentes de presença da guilda.
- O botão Copiar Diagnóstico inclui todos esses campos para suporte rápido.

### Party e heartbeat
- Mantém a deduplicação de `party_snapshot` da v0.5.2.
- Mantém `client_heartbeat` aproximadamente a cada 15 segundos.
- Heartbeat continua sendo saúde da fonte, nunca estado autoritativo da party ou de jogador.

### Guild Presence
- Mostra quantos eventos foram observados na sessão e quantos jogadores distintos o client enxergou.
- Não altera o parser Photon e não cria censo artificial.

### Compatibilidade
- Mantém ativação por código, outbox persistente, CTA context, loot, combate e updater assinado.

### Teste desta versão
Com o Albion aberto, confirme que heartbeat permanece recente, party_snapshot só muda quando a composição muda, Guild Presence cresce conforme eventos são observados e o painel de atividade recente é atualizado.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.5.3 usa a assinatura
...
Write-Host "==================================================" -ForegroundColor DarkGray
Write-Host ""
Write-Host "TESTE CONTROLADO DO UPDATER v0.5.2 -> v0.5.3" -ForegroundColor Yellow
Write-Host ""
Write-Host "1. NÃO instale a v0.5.3 manualmente na máquina de teste." -ForegroundColor Cyan
...
Write-Host "4. Confirme que a janela oferece a v0.5.3." -ForegroundColor Cyan
Write-Host "5. Atualize e confirme download, fechamento, instalação e relançamento." -ForegroundColor Cyan
Write-Host "6. Após reabrir, confirme v0.5.3 e valide o novo painel de diagnóstico operacional." -ForegroundColor Cyan
Write-Host ""
