...
$WorkRoot = Join-Path $env:TEMP "icc-v050"
...
Write-Host "Publicação da v0.5.0 experimental com Guild Presence Collector." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v050.iss"
...
if ($projectText -notmatch '<ApplicationVersion>0\.4\.9\.0</ApplicationVersion>') {
    throw "csproj não está em v0.5.0."
}
if ($assemblyText -notmatch 'AssemblyFileVersion\("0\.4\.9\.0"\)') {
    throw "AssemblyFileVersion não está em v0.5.0."
}
if ($assemblyText -notmatch 'AssemblyInformationalVersion\("0\.4\.9"\)') {
    throw "AssemblyInformationalVersion não está em v0.5.0."
}
...
if ($bridgeText -notmatch 'version = "0.5.0"') {
    throw "Telemetria ainda não anuncia v0.5.0."
}
$UpdaterFile = Join-Path $ProjectDir "Updater\AutoUpdateController.cs"
$updaterText = [System.IO.File]::ReadAllText($UpdaterFile)
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.5.0*") {
    throw "Executável gerado não tem FileVersion 0.5.0."
}
...
$notes = @"
### Guild Presence Collector experimental
...
### Teste desta versão
A v0.5.0 deve ser recebida automaticamente pela v0.4.9. Após atualizar, deixe o Albion aberto e use o cliente normalmente para que o War Room receba os probes de presença da guilda.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.5.0 usa a assinatura
...
if (Test-Path -LiteralPath $ReadmeFile) {
    ...
    $readmeText = $readmeText.Replace("> **Versão publicada atual:** v0.4.8", "> **Versão publicada atual:** v0.5.0")
...
Write-Host "==================================================" -ForegroundColor DarkGray
Write-Host ""
Write-Host "TESTE CONTROLADO DO UPDATER v0.4.9 -> v0.5.0" -ForegroundColor Yellow
Write-Host ""
Write-Host "1. NÃO instale a v0.5.0 manualmente na máquina de teste." -ForegroundColor Cyan
...
Write-Host "4. Confirme que a janela oferece a v0.5.0." -ForegroundColor Cyan
Write-Host "5. Atualize e confirme download, fechamento, instalação e relançamento." -ForegroundColor Cyan
Write-Host "6. Após reabrir, confirme v0.5.0 e deixe o Albion aberto para coletar Guild Presence probes." -ForegroundColor Cyan
Write-Host ""
