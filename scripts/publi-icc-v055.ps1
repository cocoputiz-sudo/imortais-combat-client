...
$WorkRoot = Join-Path $env:TEMP "icc-v055"
...
Write-Host "Script revision: v055-r1 (zerg death observer)" -ForegroundColor DarkGray
Write-Host "Publicação da v0.5.5 com observação de kills/deaths de terceiros para a zerg." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v055.iss"
...
if ($projectText -notmatch '<ApplicationVersion>\s*0\.5\.4\.0\s*</ApplicationVersion>') {
    throw "csproj não está em v0.5.5."
}
if ($assemblyText -notmatch 'AssemblyFileVersion\s*\(\s*"0\.5\.4\.0"\s*\)') {
    throw "AssemblyFileVersion não está em v0.5.5."
}
if ($assemblyText -notmatch 'AssemblyInformationalVersion\s*\(\s*"0\.5\.4"\s*\)') {
    throw "AssemblyInformationalVersion não está em v0.5.5."
}
...
if ($bridgeText -notmatch 'ClientVersion\s*=\s*"0\.5\.4"') {
    throw "ClientVersion da telemetria não está em v0.5.5."
}
if ($bridgeText -notmatch 'Type\s*=\s*"client_heartbeat"') {
    throw "client_heartbeat não encontrado."
}
if ($bridgeText -notmatch 'HeartbeatInterval\s*=\s*TimeSpan\.FromSeconds\(15\)') {
    throw "Cadência de 15s do client_heartbeat não encontrada."
}
if ($bridgeText -notmatch 'LastPartySnapshotPayloads') {
    throw "Deduplicação de party_snapshot não encontrada."
}
if ($bridgeText -notmatch 'string\.Join\("\|",\s*normalized\)') {
    throw "Fingerprint estável da party não encontrado."
}
if ($bridgeText -notmatch 'lastPartySnapshotAt' -or $bridgeText -notmatch 'lastPartyMemberCount') {
    throw "Metadados do último party_snapshot não encontrados no heartbeat."
}
if ($bridgeText -notmatch 'GuildPresenceDistinctPlayers') {
    throw "Diagnóstico local não expõe cobertura de Guild Presence."
}
if ($bridgeText -notmatch 'PartySnapshotDeduplicatedCount') {
    throw "Diagnóstico local não expõe deduplicação de party_snapshot."
}
if ($bridgeText -notmatch 'RecentActivity') {
    throw "Log de atividade recente não encontrado no bridge."
}
if ($windowText -notmatch 'ImortaisDiagnosticsHeartbeatText') {
    throw "Diagnóstico visual de heartbeat não encontrado."
}
if ($windowText -notmatch 'ImortaisDiagnosticsPartyText') {
    throw "Diagnóstico visual de party_snapshot não encontrado."
}
if ($windowText -notmatch 'ImortaisDiagnosticsGuildText') {
    throw "Diagnóstico visual de Guild Presence não encontrado."
}
if ($windowText -notmatch 'ImortaisDiagnosticsActivityText') {
    throw "Painel de atividade recente não encontrado."
}
if ($bridgeText -notmatch '\["cluster"\]') {
    throw "Telemetria de combate não inclui cluster."
}
if ($bridgeText -notmatch 'player_death_observed' -or $bridgeText -notmatch 'ObservePlayerDeath') {
    throw "Observer bruto de deaths não encontrado no bridge."
}
if ($diedEventHandlerText -notmatch 'ImortaisEventBridge\.ObservePlayerDeath') {
    throw "DiedEventHandler não encaminha deaths observadas ao War Room."
}
if ($windowCodeText -notmatch 'Guild Presence') {
    throw "Diagnóstico v0.5.3 não foi preservado."
}
$UpdaterFile = Join-Path $ProjectDir "Updater\AutoUpdateController.cs"
$updaterText = [System.IO.File]::ReadAllText($UpdaterFile)
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.5.5*") {
    throw "Executável gerado não tem FileVersion 0.5.5."
}
...
$notes = @"
### Kills e mortes da zerg
- O Combat Client passa a observar o DiedEvent bruto antes do filtro de jogador local.
- Cada observação envia killer, vítima, guilds, ObjectIds, mapa e flag lethal ao War Room.
- Isso permite contar kills e mortes de membros de I M O R T A I S, IMORTAIS 2 e IMORTAIS ACADEMY mesmo quando o jogador que matou/morreu não está rodando o Combat Client.

### Cobertura com poucos scanners
- A arquitetura é feita para até 5 observers.
- Múltiplos scanners aumentam cobertura; o War Room deduplica o mesmo killer/victim/mapa em uma janela curta.
- O comportamento original do Statistics Analysis continua preservado.

### Compatibilidade
- Mantém mapa/cluster v0.5.4, heartbeat, Guild Presence, party dedup, CTA context, loot, diagnóstico e outbox persistente.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.5.5 usa a assinatura
