...
$WorkRoot = Join-Path $env:TEMP "icc-v052"
...
Write-Host "Script revision: v052-r1 (heartbeat + party dedup)" -ForegroundColor DarkGray
Write-Host "Publicação da v0.5.2 com party deduplicada e client heartbeat." -ForegroundColor DarkGray
Write-Host ""
...
$IssFile = Join-Path $InstallerDir "icc-v052.iss"
...
if ($bridgeText -notmatch 'ClientVersion\s*=\s*"0\.5\.2"') {
    throw "ClientVersion da telemetria não está em v0.5.2."
}
...
if ($exeInfo.VersionInfo.FileVersion -notlike "0.5.2*") {
    throw "Executável gerado não tem FileVersion 0.5.2."
}
...
$notes = @"
### Party state eficiente
- `party_snapshot` agora é deduplicado por composição normalizada.
- Snapshots idênticos consecutivos não entram novamente no outbox.
- Uma entrada ou saída da party gera um novo snapshot autoritativo.

### Client heartbeat
- Novo evento `client_heartbeat` aproximadamente a cada 15 segundos.
- Informa device, player, versão, CTA atual, conexão, outbox e metadados do último snapshot.
- Inclui `gameDetected` quando disponível.
- O heartbeat NÃO transporta a lista de membros e NÃO altera o estado da party.

### Regra de estado
- Silêncio de party não significa party vazia.
- Heartbeat permite distinguir party estável de client sem comunicação.
- Todos os eventos continuam usando o mesmo outbox durável e `/api/telemetry/ingest`.

### Compatibilidade
- Mantém ativação por código, Guild Presence Collector, CTA context, updater assinado e demais eventos existentes.

### Teste desta versão
Atualize da v0.5.1 para v0.5.2. Com o Albion aberto, confirme heartbeats em cadência e que uma party sem alterações não gera snapshots repetidos.
"@
if ($releaseExists) {
...
# Mantemos o appcast canonicalizado em LF. A v0.5.2 usa a assinatura
...
Write-Host "==================================================" -ForegroundColor DarkGray
Write-Host ""
Write-Host "TESTE CONTROLADO DO UPDATER v0.5.1 -> v0.5.2" -ForegroundColor Yellow
Write-Host ""
Write-Host "1. NÃO instale a v0.5.2 manualmente na máquina de teste." -ForegroundColor Cyan
Write-Host "2. Confirme que o Combat Client instalado está em v0.5.1." -ForegroundColor Cyan
Write-Host "3. Abra a v0.5.1 e use Verificar atualização." -ForegroundColor Cyan
Write-Host "4. Confirme que a janela oferece a v0.5.2." -ForegroundColor Cyan
Write-Host "5. Atualize e confirme download, fechamento, instalação e relançamento." -ForegroundColor Cyan
Write-Host "6. Após reabrir, confirme v0.5.2 e deixe o Albion aberto para validar heartbeat e party dedup." -ForegroundColor Cyan
Write-Host ""
